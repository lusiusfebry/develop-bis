...
export default function ImportPage() {
    ...
    const previewMutation = useMutation({
        ...
        onError: (error) => {
            ...
            toast.error('Gagal membaca file atau format tidak sesuai.');
        }
    });
    ...
    const executeMutation = useMutation({
        ...
        onError: (error) => {
            ...
            toast.error('Gagal mengeksekusi import data.');
        }
    });
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h1 className="text-2xl font-bold text-foreground">Import Data Karyawan</h1>
            </div>
                ...
                <div className="glass-card p-6 space-y-6">
                    <div className="flex justify-end">
                        <button
                            ...
                            className="flex items-center space-x-2 px-4 py-2 bg-card border border-border/50 hover:bg-accent hover:border-border text-foreground rounded-2xl transition-all duration-300 text-sm shadow-sm active:scale-95 cursor-pointer"
                        >
                    ...
                    <div
                        ...
                        onDrop={handleDrop}
                        className="border-2 border-dashed border-border hover:border-white/40 rounded-2xl p-12 flex flex-col items-center justify-center bg-card hover:bg-accent transition-all duration-300 cursor-pointer"
                    ...
                    <div className="flex justify-end pt-4">
                        <button
                            ...
                            className="px-6 py-2.5 bg-accent/50 border border-border/50 hover:bg-accent/80 hover:border-border/80 disabled:opacity-50 disabled:cursor-not-allowed text-foreground rounded-2xl flex items-center space-x-2 font-medium transition-all duration-300 shadow-sm active:scale-95 cursor-pointer"
                        >
            ...
            {step === 2 && previewData && (
                <div className="space-y-6">
                    ...
                    <div className="glass-card p-0 overflow-hidden">
                        <div className="overflow-x-auto max-h-[500px] overflow-y-auto">
                            <table className="w-full text-left text-sm text-gray-300">
                                <thead className="text-xs text-gray-400 uppercase bg-black/40 backdrop-blur-md border-b border-border/50 sticky top-0 z-10">
                                    <tr>
                                ...
                                <tbody className="divide-y divide-white/5">
                                    {previewData.rows.map((row) => (
                                        <tr
                                            key={row.rowNumber}
                                            className={`hover:bg-card ${!row.isValid ? 'bg-red-500/5' : row.warnings.length > 0 ? 'bg-amber-500/5' : ''}`}
                                        >
                    ...
                    <div className="flex justify-between pt-4">
                        <button
                            onClick={() => setStep(1)}
                            className="px-6 py-2.5 bg-card border border-border/50 hover:bg-accent hover:border-border text-foreground rounded-2xl flex items-center space-x-2 font-medium transition-all duration-300 shadow-sm active:scale-95 cursor-pointer"
                        >
                ...
                <div className="space-y-6">
                    {executeMutation.isPending ? (
                        ...
                        <div className="space-y-6">
                            <div className="glass-card p-6">
                                <div className="text-center mb-8">
                                    ...
                                    <h2 className="text-2xl font-bold text-foreground mb-2">Import Selesai</h2>
                                    <p className="text-gray-400">Proses import data karyawan telah selesai dieksekusi.</p>
                                </div>

                                <div className="grid grid-cols-3 gap-4 mb-8">
                                    <div className="bg-card p-4 rounded-xl border border-white/5 flex flex-col items-center justify-center text-center">
                                ...
                                <div className="flex justify-center mt-8">
                                    <button
                                        onClick={handleReset}
                                        className="px-6 py-2.5 bg-accent/50 border border-border/50 hover:bg-accent/80 hover:border-border/80 text-foreground rounded-2xl flex items-center space-x-2 font-medium transition-all duration-300 shadow-sm active:scale-95 cursor-pointer"
                                    >
                        ...
                        <div className="glass-card p-12 flex flex-col items-center justify-center space-y-4">
                            ...
                            <button onClick={handleReset} className="px-4 py-2 mt-4 bg-card border border-border/50 hover:bg-accent hover:border-border rounded-2xl text-foreground transition-all duration-300 shadow-sm active:scale-95 cursor-pointer">
                                Coba Lagi
                            </button>
                        </div>
                    )}
